    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                ...
                print("clique")
                pygame.draw.rect(win, (0,0,0), (pos[0]*50+buffer, pos[1]*50+buffer, 50-2*buffer, 50-2*buffer))
                pygame.display.update()
                #print(pos[0]//50) #dá o numero do quadrado de cada linha quando se clica
            if event.type == pygame.QUIT:
                pygame.quit()
                return
main()