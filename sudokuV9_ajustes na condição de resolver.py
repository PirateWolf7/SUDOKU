...
solucao=0
def main():
    global dificuldade, solucao, solution, bg_color_dif1, bg_color_dif2, bg_color_dif3, bg_color_dif4, bg_color_dif5, bg_color_dif6, bg_color_dif7, bg_color_dif8, bg_color_dif9
    pygame.init()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if (retangulo_novo[0] <
                    ...
                    = mouse[0] <= retangulo_novo[0]+retangulo_novo[2]) and (retangulo_novo[1] <= mouse[1] <= retangulo_novo[1]+retangulo_novo[3]):
                    # novo_sudoku()
                    solucao = 0
                    main() #vai ao main e cria uma nova grid com novos numeros
                if (solucao==0 and dificuldade != 1 and retangulo_solucao[0] <= mouse[0] <= retangulo_solucao[0]+retangulo_solucao[2]) and (retangulo_solucao[1] <= mouse[1] <= retangulo_solucao[1]+retangulo_solucao[3]):
                    print(dificuldade)
                    solucao = 1
                    solution = sudokum.solve(grid)
                    print(grid)
                    print(solution[1]) #imprime só a solução sem o True antes
                    main()
